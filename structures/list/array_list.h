...
namespace structures
{
	...
	inline Structure& ArrayList<T>::assign(Structure& other)
	{
		if (this != &other) {
			...
			delete array_;
			array_ = new Array<T>(*otherArray.array_);
		...
		if (this != &other) {
			...
			if (otherArray->size_ == size_) {
				for (size_t i = 0; i < size_; i++) {
					if (otherArray->array_->at(i) != array_->at(i)) {
						return false;
					}
				}
				return true;
	...
	{
		Utils::rangeCheckExcept(index, size_, "Index out of bounds!");
	...
	inline T ArrayList<T>::removeAt(int index)
	{
		T data = at(index);
		if (index != size_ - 1) {
			array_->copy(*array_, index + 1, *array_, index, size_ - index - 1);
		}
		size_--;
		return data;
	}
	...
	{
		ArrayListIterator* otherALI = dynamic_cast<ArrayListIterator*>(&other);
		return otherALI == nullptr || position_ != otherALI->position_ || arrayList_ != otherALI->arrayList_;
	}
